...
class Knob @JvmOverloads constructor(
    ...
    var knobSize: Int by interceptedUIProperty(0, updateMeasurementsOnChange = true) { value ->
        max(
            0,
            value
        )
    ...
    var fillStartAngle: Float by interceptedUIProperty(DEFAULT_FILL_START_ANGLE) { value ->
        if (value.isFinite()) max(
            0F,
            value
        ) % 360F else DEFAULT_FILL_START_ANGLE
    }
    var fillSweepAngle: Float by interceptedUIProperty(DEFAULT_FILL_SWEEP_ANGLE) { value ->
        if (value.isFinite()) min(
            360F,
            max(0F, value)
        ) else DEFAULT_FILL_SWEEP_ANGLE
    }

    var trackStrokeColor: Int by uiProperty(DEFAULT_TRACK_STROKE_COLOR)
    var trackStrokeWidth: Int by interceptedUIProperty(
        0,
        updateMeasurementsOnChange = true
    ) { value -> max(0, value) }
    var trackStrokeCap: Paint.Cap by uiProperty(
        DEFAULT_TRACK_STROKE_CAP,
        updateMeasurementsOnChange = true
    )
    var trackStrokeOffset: Int by uiProperty(0, updateMeasurementsOnChange = true)

    var stepCount: Int by interceptedUIProperty(DEFAULT_STEP_COUNT) { value ->
        max(
            2,
            value
        ).also { post { progress = progress } }
    }
    ...
    var stepAnchorX: Float by interceptedUIProperty(DEFAULT_STEP_ANCHOR_X) { value ->
        if (value.isFinite()) min(
            1F,
            max(0F, value)
        ) else DEFAULT_STEP_ANCHOR_X
    }
    var stepAnchorY: Float by interceptedUIProperty(
        DEFAULT_STEP_ANCHOR_Y,
        updateMeasurementsOnChange = true
    ) { value -> if (value.isFinite()) min(1F, max(0F, value)) else DEFAULT_STEP_ANCHOR_Y }

    var substepCount: Int by interceptedUIProperty(DEFAULT_SUBSTEP_COUNT) { value ->
        max(
            2,
            value
        ).also { post { progress = progress } }
    ...
    var substepAnchorX: Float by interceptedUIProperty(DEFAULT_SUBSTEP_ANCHOR_X) { value ->
        if (value.isFinite()) min(
            1F,
            max(0F, value)
        ) else DEFAULT_SUBSTEP_ANCHOR_X
    }
    var substepAnchorY: Float by interceptedUIProperty(
        DEFAULT_SUBSTEP_ANCHOR_Y,
        updateMeasurementsOnChange = true
    ) { value -> if (value.isFinite()) min(1F, max(0F, value)) else DEFAULT_SUBSTEP_ANCHOR_Y }

    var progressMode: ProgressMode by reactiveUIProperty(DEFAULT_PROGRESS_MODE) {
        progress = progress
    }
    var maxProgress: Float by interceptedUIProperty(DEFAULT_MAX_PROGRESS) { value ->
        (if (value.isFinite()) max(
            0F,
            value
        ) else DEFAULT_MAX_PROGRESS).also { progress = min(progress, it) }
    }
    var progress: Float by interceptedUIProperty(DEFAULT_PROGRESS) { value ->
        ...
        when (progressMode) {
            ...
            ProgressMode.STEP -> (maxProgress / (stepCount - (if (progressSweepAngle < 360F) 1 else 0))).let { progressStep ->
                progressStep * floor(
                    (progressValue + (progressStep * 0.5F)) / progressStep
                )
            }

            ProgressMode.SUBSTEP -> (maxProgress / (substepCount - (if (progressSweepAngle < 360F) 1 else 0))).let { progressStep ->
                progressStep * floor(
                    (progressValue + (progressStep * 0.5F)) / progressStep
                )
            }
    ...
    var progressStrokeWidth: Int by interceptedUIProperty(
        0,
        updateMeasurementsOnChange = true
    ) { value -> max(0, value) }
    ...
    var progressStartAngle: Float by interceptedUIProperty(
        DEFAULT_PROGRESS_START_ANGLE,
        updateMeasurementsOnChange = true
    ) { value -> if (value.isFinite()) max(0F, value) % 360F else DEFAULT_PROGRESS_START_ANGLE }
    var progressSweepAngle: Float by interceptedUIProperty(
        DEFAULT_PROGRESS_SWEEP_ANGLE,
        updateMeasurementsOnChange = true
    ) { value -> if (value.isFinite()) min(360F, max(0F, value)) else DEFAULT_PROGRESS_SWEEP_ANGLE }
    ...
    var thumbAnchorX: Float by interceptedUIProperty(DEFAULT_THUMB_ANCHOR_X) { value ->
        if (value.isFinite()) min(
            1F,
            max(0F, value)
        ) else DEFAULT_THUMB_ANCHOR_Y
    }
    var thumbAnchorY: Float by interceptedUIProperty(
        DEFAULT_THUMB_ANCHOR_Y,
        updateMeasurementsOnChange = true
    ) { value -> if (value.isFinite()) min(1F, max(0F, value)) else DEFAULT_THUMB_ANCHOR_Y }
    var thumbRotation: Boolean by uiProperty(DEFAULT_THUMB_ROTATION)

    var touchThreshold: Int by interceptedUIProperty(
        0,
        updateMeasurementsOnChange = true
    ) { value -> max(0, value) }
    ...
    init {
        ...
        knobSize = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            DEFAULT_KNOB_SIZE_DP.toFloat(),
            displayMetrics
        ).toInt()

        trackStrokeWidth = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            DEFAULT_TRACK_STROKE_WIDTH_DP.toFloat(),
            displayMetrics
        ).toInt()
        trackStrokeOffset = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            DEFAULT_TRACK_STROKE_OFFSET_DP.toFloat(),
            displayMetrics
        ).toInt()

        progressStrokeWidth = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            DEFAULT_PROGRESS_STROKE_WIDTH_DP.toFloat(),
            displayMetrics
        ).toInt()
        progressStrokeOffset = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            DEFAULT_PROGRESS_STROKE_OFFSET_DP.toFloat(),
            displayMetrics
        ).toInt()

        thumbOffset = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            DEFAULT_THUMB_OFFSET_DP.toFloat(),
            displayMetrics
        ).toInt()

        stepOffset = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            DEFAULT_STEP_OFFSET_DP.toFloat(),
            displayMetrics
        ).toInt()

        touchThreshold = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            DEFAULT_TOUCH_THRESHOLD_DP.toFloat(),
            displayMetrics
        ).toInt()
        touchOffset = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP,
            DEFAULT_TOUCH_OFFSET_DP.toFloat(),
            displayMetrics
        ).toInt()
        ...
        context.obtainStyledAttributes(attrs, R.styleable.Knob, 0, 0).apply {
            ...
            trackStrokeWidth =
                getDimensionPixelSize(R.styleable.Knob_trackStrokeWidth, trackStrokeWidth)
            trackStrokeCap = Paint.Cap.values()[getInteger(
                R.styleable.Knob_trackStrokeCap,
                trackStrokeCap.ordinal
            )]
            trackStrokeOffset =
                getDimensionPixelSize(R.styleable.Knob_trackStrokeOffset, trackStrokeOffset)
            ...
            progressMode = ProgressMode.values()[getInteger(
                R.styleable.Knob_progressMode,
                DEFAULT_PROGRESS_MODE.ordinal
            )]
            maxProgress = getFloat(R.styleable.Knob_maxProgress, maxProgress)
            progress = getFloat(R.styleable.Knob_progress, progress)
            progressStrokeColor =
                getColor(R.styleable.Knob_progressStrokeColor, progressStrokeColor)
            progressStrokeWidth =
                getDimensionPixelSize(R.styleable.Knob_progressStrokeWidth, progressStrokeWidth)
            progressStrokeCap = Paint.Cap.values()[getInteger(
                R.styleable.Knob_progressStrokeCap,
                progressStrokeCap.ordinal
            )]
            progressStrokeOffset =
                getDimensionPixelSize(R.styleable.Knob_progressStrokeOffset, progressStrokeOffset)
            ...
            debugDrawTouchRadius =
                getBoolean(R.styleable.Knob_debug_drawTouchRadius, debugDrawTouchRadius)
            debugDrawTouchArea =
                getBoolean(R.styleable.Knob_debug_drawTouchArea, debugDrawTouchArea)
        }.recycle()
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (widthMode != MeasureSpec.EXACTLY) {
            val preferredWidth = drawingBounds.width().toInt() + paddingLeft + paddingRight

            width =
                if (widthMode == MeasureSpec.AT_MOST) min(width, preferredWidth) else preferredWidth
        }

        if (heightMode != MeasureSpec.EXACTLY) {
            val preferredHeight = drawingBounds.height().toInt() + paddingTop + paddingBottom

            height = if (heightMode == MeasureSpec.AT_MOST) min(
                height,
                preferredHeight
    ...
    private fun onTouch() {
        val angle = (180 + Math.toDegrees(
            atan2(
                drawingBoundsCenter.y - activeTouchPoint.y,
                drawingBoundsCenter.x - activeTouchPoint.x
            ).toDouble()
        ).toFloat()) % 360
        ...
        if (sweep < progressSweepAngle) {
            ...
            progress =
                if (sweep - progressSweepAngle < (360 - progressSweepAngle) * 0.5F) maxProgress else 0F
        }
    ...
    private fun drawDebug(canvas: Canvas) {
        ...
        if (debugDrawTouchRadius) {
            if (!activeTouchPoint.x.isNaN() && !activeTouchPoint.y.isNaN()) {
                ...
                canvas.drawLine(
                    drawingBoundsCenter.x,
                    drawingBoundsCenter.y,
                    activeTouchPoint.x,
                    activeTouchPoint.y,
                    paint
                )
    ...
    private fun drawFill(canvas: Canvas) {
        ...
        canvas.drawArc(
            arcRect.apply { set(0F, 0F, knobSize.toFloat(), knobSize.toFloat()) },
            fillStartAngle,
            fillSweepAngle,
            false,
            paint
    ...
    private fun drawTrack(canvas: Canvas) {
        ...
        canvas.drawArc(
            arcRect.apply {
                set(
                    0F, 0F, knobSize + trackStrokeOffset * 2F, knobSize + trackStrokeOffset * 2F
                )
            }, progressStartAngle, progressSweepAngle, false, paint
        )
        canvas.restore()
    }
    ...
    private fun drawProgress(canvas: Canvas) {
        ...
        canvas.save()
        canvas.translate(
            drawingOffset.x - progressStrokeOffset,
            drawingOffset.y - progressStrokeOffset
        )
        canvas.drawArc(
            arcRect.apply {
                set(
            ...
            }, progressStartAngle, progressSweepAngle * (progress / maxProgress), false, paint
        )
        canvas.restore()
    }
    ...
    private fun drawThumb(canvas: Canvas) {
        thumbDrawable?.let { drawable ->
            trackPathMeasure.getPosTan(
                (progress / maxProgress) * trackPathMeasure.length,
                pathPosition,
                pathTangent
            )
            ...
            canvas.concat(transformMatrix.apply {
                ...
                if (!thumbRotation) {
                    postRotate(
                        -rotation, drawable.bounds.width() * 0.5F, drawable.bounds.height() * 0.5F
                    )
                }
                ...
                postRotate(
                    rotation, pathPosition[0], pathPosition[1]
                )
            })
    ...
    private fun drawSteps(
        ...
        if (count >= 2) {
            ...
            repeat(count) { iteration ->
                trackPathMeasure.getPosTan(
                    (iteration * stepProgress / maxProgress) * trackPathMeasure.length,
                    pathPosition,
                    pathTangent
                )
    ...
    private fun updateMeasurements() {
        ...
        val maxOffset = maxOf(
            ...
            thumbDrawable?.let { drawable -> drawable.bounds.height() * thumbAnchorY + thumbOffset }
                ?: 0F,
            stepDrawable?.let { drawable -> drawable.bounds.height() * stepAnchorY + stepOffset }
                ?: 0F,
            substepDrawable?.let { drawable -> drawable.bounds.height() * substepAnchorY + substepOffset }
                ?: 0F,
        ...
        if (_touchThreshold > 0) {
            touchPath.addArc(arcRect.apply {
                set(
                    -_touchThreshold - touchOffset,
                    -_touchThreshold - touchOffset,
                    knobSize.toFloat() + _touchThreshold + touchOffset,
                    knobSize.toFloat() + _touchThreshold + touchOffset
                )
            }, 0F, 360F)
            touchPath.op(
                Path().apply {
                    addArc(arcRect.apply {
                        set(
                            _touchThreshold - touchOffset,
                            _touchThreshold - touchOffset,
                            knobSize.toFloat() - _touchThreshold + touchOffset,
    ...
    private open class UIPropertyDelegate<T>(
        private var value: Any? = null, private val updateMeasurementsOnChange: Boolean = false
